...
class KkbPayment {
    // -----------------------------------------------------------------------------------------------
    public static function process_XML($filename,$reparray) {
...
// -----------------------------------------------------------------------------------------------
    public static function process_request($order_id,$currency_code,$amount,$config_file,$b64=true) {
        ...
        $result = self::process_XML($config['XML_TEMPLATE_FN'],$request);
...
// -----------------------------------------------------------------------------------------------
    ...
    public static function process_response($response,$config_file) {
        ...
        if (in_array("DOCUMENT",$result)){
            $kkb = new KkbSign();
            $kkb->invert();
            $data = KkbPayment::split_sign($response,"BANK");
